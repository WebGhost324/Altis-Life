...
{
    case west: {
        if (_item in ["U_Rangemaster","U_B_CombatUniform_mcam_worn","U_B_CombatUniform_mcam","B_Bergen_blk"]) then {
    ...
    case independent: {
        if (_item in ["U_C_Scientist","B_TacticalPack_blk"]) then {
            [] call life_fnc_playerSkins;
        };
    };
	case east: {
        if (_item in ["U_C_Scientist","U_Rangemaster","B_Kitbag_cbr"]) then {
            [] call life_fnc_playerSkins;
        };
    };
};
